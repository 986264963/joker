// 页面下拉模型与面板绑定（仅3个新模型，清除原版4个旧模型逻辑）
...
// API密钥本地存储逻辑（修复Bearer编码报错，兼容有无Bearer两种密钥）
...
document.getElementById("btnFluxTxt").onclick = async function(){
  ...
  try{
    ...
    if(json.data) json.data.forEach(item=>outputDiv.innerHTML += `<div><img style="max-width:100%;" src="${item.url}" /></div>`);
    else outputDiv.innerHTML += `<pre>${JSON.stringify(json,null,2)}</pre>`;
  }catch(err){
...
// 2. FLUX.1-Kontext-dev 图生图（路径已修改 /api/images/generations）
...
// 3. Wan2.1-I2V-A14B 图生视频（路径已修改 /api/videos/generations）
document.getElementById("btnWanRun").onclick = async function(){
  ...
  try{
    ...
    const json = await resp.json();
    statusBadge.textContent = "准备就绪";
    if(json.data?.[0]?.url) outputDiv.innerHTML += `<div><video controls style="max-width:100%;"><source src="${json.data[0].url}"></video></div>`;
    else outputDiv.innerHTML += `<pre>${JSON.stringify(json,null,2)}</pre>`;
  }catch(err){
    statusBadge.textContent = "请求异常";
    outputDiv.innerHTML += `<pre>错误：${err.message}</pre>`;
