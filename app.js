...
const XL_MODEL_LIST = {
  "Juggernaut-XL（外景人体全能）": "scenario-labs/juggernaut_reborn",
...
// -------- SDXL 写实人像：Juggernaut / CyberRealistic --------
async function runXLImage() {
  const apiKey = getApiKey();
  rememberKeyMaybe();

  const prompt = $("xlPrompt").value.trim();
  const negativePrompt = $("xlNeg").value.trim();

  if (!prompt) {
    throw new Error("请输入正向提示词 / Please input positive prompt");
  }

  const modelName = $("xlModelSel").value;
  const modelId = XL_MODEL_LIST[modelName];

  const n = clampInt($("xlN").value, 1, 4, 1);
  const [w, h] = XL_RESOLUTIONS[$("xlRes").value];
  const size = `${w}x${h}`;

  const steps = clampInt($("xlSteps").value, 10, 50, 28);
  const guidance = clampFloat($("xlGuidance").value, 1, 15, 7.0);
  const seedRaw = $("xlSeed").value.trim();
  const seed = seedRaw === "" ? null : clampInt(seedRaw, 1, 999999999, null);

  setStatus(`SDXL ${modelName} 创建任务...`);
  ...
  if (!res.ok) {
    setStatus(`SDXL ${modelName} 创建失败`, "err");
    addOutputItem({
      title: `SDXL 生成失败`,
  ...
  if (!taskId) {
    setStatus("SDXL 无 task_id 返回", "err");
    addOutputItem({ title: "SDXL 任务缺失 task_id", rawJson: j });
    throw new Error("Task ID missing");
  }

  addOutputItem({
    title: `SDXL ${modelName} 任务创建成功`,
    meta: `task_id=${taskId} • model=${modelId} • size=${size} • steps=${steps} • guidance=${guidance}`,
    rawJson: j,
  });

  setStatus(`SDXL ${modelName} 轮询渲染中`);

  const pollResult = await pollTask(taskId, apiKey, {
    intervalMs: 8000,
    timeoutMs: 30 * 60 * 1000,
    onTick: (info) => {
      setStatus(waitingStatusText(`SDXL ${modelName}`, info.tick, info.elapsedMs, `task=${taskId.slice(0,8)}`));
    }
  });

  const st = pollResult.status;
  const raw = pollResult.raw || {};

  if (st !== "success") {
    setStatus(`SDXL 任务：${st}`, st === "failed" ? "err" : "info");
    addOutputItem({
      title: `SDXL 任务结束：${st}`,
      meta: `task_id=${taskId}`,
      rawJson: raw
  ...
  const imgs = raw?.output?.images || [];

  if (!imgs.length) {
    addOutputItem({ title: "SDXL 渲染成功但无图片", meta: `task_id=${taskId}`, rawJson: raw });
    setStatus("SDXL 完成", "ok");
    return;
  }

  for (let i = 0; i < imgs.length; i++) {
    const url = imgs[i].url;
    const blobInfo = await fetchAsBlob(url, "image");
    const imgDom = document.createElement("img");
    imgDom.src = blobInfo.objUrl;

    addOutputItem({
      title: `SDXL 输出 #${i+1} | ${modelName}`,
      meta: `size=${size}, steps=${steps}`,
      element: imgDom,
      download: { href: blobInfo.objUrl, filename: `sdxl_${nowTs()}_${i+1}.png` },
      openUrl: url
    });
  }

  setStatus(`SDXL ${modelName} 全部渲染完成`, "ok");
}
...
function initUi() {
  ...
  // SDXL 分辨率
  const xlRes = $("xlRes");
  for (const k of Object.keys(XL_RESOLUTIONS)) {
    const o = document.createElement("option");
    o.value = k; o.textContent = k;
    xlRes.appendChild(o);
  }
  xlRes.value = "9:16 (576x1024)";

  // SDXL 模型选择
  const xlModelSel = $("xlModelSel");
  for (const label of Object.keys(XL_MODEL_LIST)) {
    const opt = document.createElement("option");
    opt.value = label;
    opt.textContent = label;
    xlModelSel.appendChild(opt);
  }
  xlModelSel.value = Object.keys(XL_MODEL_LIST)[0];
  ...
  $("btnXLRun").onclick = async () => {
    try { await runXLImage(); }
    catch (e) { addOutputItem({ title: "SDXL 人像模型错误 / Error", meta: String(e) }); }
  };
