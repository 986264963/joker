...
// - Downloads images/via /dl?url=... (Pages Function) to avoid cross-origin blocks.
...
function showPanel(model) {
  ...
  $("panelFlux").style.display = model === "flux‑dev" ? "block" : "none";
}
...
// ===================== flux-dev 人像图生图 新增完整逻辑 =====================
let fluxBase64Image = null;
document.getElementById("fluxImg")?.addEventListener("change", async function(e) {
  const file = e.target.files[0];
  if (!file) {
    fluxBase64Image = null;
    return;
  }
  const reader = new FileReader();
  reader.readAsDataURL(file);
  reader.onload = ev => {
    fluxBase64Image = ev.target.result;
  };
});
...
async function runFlux() {
  ...
  if (!fluxBase64Image) throw new Error("请先上传一张参考原图！");

  const prompt = $("fluxPrompt").value.trim();
  if (!prompt) throw new Error("请输入正向提示词！");
  const negative_prompt = $("fluxNeg").value.trim();
  const denoising_strength = clampFloat($("fluxDenoise").value, 0.1, 1, 0.75);
  const n = clampInt($("fluxN").value, 1, 4, 1);
  const w = clampInt($("fluxW").value, 256, 2048, 896);
  const h = clampInt($("fluxH").value, 256, 2048, 1152);
  const size = `${w}x${h}`;

  setStatus("flux‑dev 人像图生图生成中...");
  const payload = {
    model: "flux‑dev",
    prompt,
    negative_prompt,
    image: fluxBase64Image,
    denoising_strength,
    n,
    size
  };
  ...
  if (!data.length) {
    addOutputItem({ title: "flux‑dev 无返回图片", rawJson: j });
    setStatus("flux‑dev 失败", "err");
    return;
  }
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (item.url) {
      ...
      for (let k = 0; k < byteStr.length; k++) arr[k] = byteStr.charCodeAt(k);
      blobInfo = {
    ...
    addOutputItem({
      title: `flux‑dev 人像输出 #${i+1}`,
      meta: `size=${size}, denoise=${denoising_strength}`,
      element: img,
      download: { href: blobInfo.objUrl, filename }
    });
  }
  setStatus("flux‑dev 生成成功", "ok");
}
// ==========================================================================
...
function buildWanFormData({
  ...
  if (watermark !== null && watermark) fd.append("watermark", "true");
  if (promptExtend !== null && promptExtend) fd.append("prompt_extend", "true");
...
function initUi() {
  ...
  $("btnFluxRun").onclick = async () => {
    try { await runFlux(); }
    catch (e) { addOutputItem({ title:"flux‑dev 错误 / Error", meta:String(e) }); }
  };
