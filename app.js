...
// -------- SDXL 写实人像 --------
async function runXlSd() {
  const apiKey = getApiKey();
  rememberKeyMaybe();

  const modelKey = $("xlModelSel").value;
  const model = XL_MODEL_LIST[modelKey];
  if (!model) throw new Error("请选择模型 / Please select a model");
  ...
  const steps = clampInt($("xlSteps").value, 1, 50, 28);
  const cfg = clampFloat($("xlCfg").value, 1, 20, 7.0);

  const seedRaw = $("xlSeed").value.trim();
  const seed = seedRaw ? clampInt(seedRaw, 0, 2147483647, null) : null;
  ...
  setStatus("SDXL 创建任务中... / Creating task...");
  ...
  const j = await readJsonSafely(res);
  if (!res.ok || !j.task_id) {
    setStatus("SDXL 创建任务失败 / Create failed", "err");
    addOutputItem({
      title: "SDXL 创建任务失败 / Create failed",
  ...
  setStatus(`SDXL 任务已创建，开始轮询... (${taskId.slice(0,8)})`);
  ...
  addOutputItem({ title: `SDXL 任务结果 task=${taskId.slice(0,8)}`, rawJson: result.raw });

  if (result.status !== "success") {
    setStatus("SDXL 生成失败 / Failed", "err");
    throw new Error(`任务失败 / Task failed: ${result.status}`);
  }

  const images = result.raw?.output?.images || [];
  if (!images.length) {
    addOutputItem({ title: "SDXL 返回无图片 / No images", rawJson: result.raw });
    setStatus("SDXL 生成失败 / Failed", "err");
    return;
  }
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    setStatus(`SDXL 下载第${i+1}张... / Downloading ${i+1}...`);
    const { objUrl } = await fetchAsBlob(fileUrl, "image");

    const img = document.createElement("img");
    img.src = objUrl;

    const filename = `sdxl-${nowTs()}-${i+1}.png`;
    addOutputItem({
      title: `SDXL 输出 #${i+1}（${modelKey}）`,
      meta: `size=${w}x${h}, steps=${steps}, cfg=${cfg}, seed=${item.seed || "随机"}`,
      element: img,
      download: { href: objUrl, filename },
    });
  }

  setStatus("SDXL 生成成功 / Success", "ok");
}
...
async function runWan() {
  ...
  const fps = clampInt($("wanFps").value, 1, 24, 24);
...
function initUi() {
  ...
  const xlModel = $("xlModelSel");
  for (const k of Object.keys(XL_MODEL_LIST)) {
    const o = document.createElement("option");
    o.value = k; o.textContent = k;
    xlModel.appendChild(o);
  }
  xlModel.value = Object.keys(XL_MODEL_LIST)[0];
  ...
  $("btnXlRun").onclick = async () => {
    try { await runXlSd(); }
    catch (e) { addOutputItem({ title:"SDXL 错误 / Error", meta:String(e) }); }
  };
